...
#     commands like plan.sh and claude -p, destroying the user's work.
...
# e.g., config.sh write claude.model "opus"
write_config() {
  local key="$1"
  local value="$2"
...
init_state() {
  ...
  jq -n \
    ...
    '{
      ...
      claude_session_id: (if $session == "null" or $session == "" then null else $session end),
